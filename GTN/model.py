...
class GTN(nn.Module):
    ...
    def norm(self, H, add=False):
        H = H.t()
        if add == False:
            H = H*(torch.eye(H.shape[0], device=H.device)==0)
        else:
            H = H*(torch.eye(H.shape[0], device=H.device)==0) + torch.eye(H.shape[0], device=H.device)
        deg = torch.sum(H, dim=1)
        deg_inv = deg.pow(-1)
        deg_inv[deg_inv == float('inf')] = 0
        deg_inv = deg_inv*torch.eye(H.shape[0], device=H.device)
