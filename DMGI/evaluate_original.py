...
def evaluate(embeds, idx_train, idx_val, idx_test, labels, args, isTest=True):
    ...
    if isTest:
        print("\t[Classification] Macro-F1: {:.4f} ({:.4f}) | Micro-F1: {:.4f} ({:.4f})".format(np.mean(macro_f1s),
                                                                                                ...
                                                                                                np.mean(micro_f1s),
                                                                                                np.std(micro_f1s)))
        accs = remove_edge_pts(np.array(accs))
        macro_f1s = remove_edge_pts(np.array(macro_f1s))
        micro_f1s = remove_edge_pts(np.array(micro_f1s))
        print("\t[Classification after filter] Macro-F1: {:.4f} ({:.4f}) | Micro-F1: {:.4f} ({:.4f})".format(np.mean(macro_f1s),
                                                                                                np.std(macro_f1s),
                                                                                                np.mean(micro_f1s),
                                                                                                np.std(micro_f1s)))
    else:
