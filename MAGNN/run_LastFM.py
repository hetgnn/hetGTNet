...
def run_model_LastFM(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    for _ in range(repeat):
        ...
        for epoch in range(num_epochs):
            ...
            # training
            net.train()
            for iteration in range(1, 1 + train_pos_idx_generator.num_iterations()):
                # forward
                t0 = time.time()
                ...
                if iteration % 100 == 0:
                    print(
                        'Epoch {:05d} | Iteration {:05d} | Train_Loss {:.4f} | Training time(s)/batch {:.4f}'.format(
                            epoch, iteration, train_loss.item(), training_time/iteration))
            # validation
            net.eval()
            val_loss = []
            with torch.no_grad():
            ...
            print('Epoch {:05d} | Val_Loss {:.4f} | Time(s)/epoch {:.4f} | Training time(s)/epoch {:.4f}'.format(
                epoch, val_loss.item(), t_end - t_start, training_time))
            # early stopping
