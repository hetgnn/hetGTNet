...
def run_model_ACM(args):
    ...
    nx_G_lists, edge_metapath_indices_lists, features_list, adjM, type_mask, labels, train_val_test_idx = load_ACM_data(args.data_path+'preprocessed/ACM_processed')
    ...
    for i in range(repeat):
        ...
        # training loop
        net.train()
        early_stopping = EarlyStopping(patience=patience, verbose=False, save_path=args.save_path+'checkpoint_ACM.pt')
        tic = time.time()
        t0 = time.time()
        training_time = 0
        ...
        # testing
        net.load_state_dict(torch.load(args.save_path+'checkpoint_ACM.pt'))
        net.eval()
        with torch.no_grad():
            logits, embeddings = net((g_lists, features_list, type_mask, edge_metapath_indices_lists), target_node_indices)
